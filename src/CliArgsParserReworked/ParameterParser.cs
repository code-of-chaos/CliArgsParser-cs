...
public class ParameterParser(Type type, IServiceProvider provider) : IParameterParser {
    ...
    // Methods
    // -----------------------------------------------------------------------------------------------------------------
    private static ImmutableDictionary<string, PropertyInfo> AssembleDict<TAttribute>(Type type) where TAttribute : Attribute, IAttruteWithName {
        return new Dictionary<string, PropertyInfo>(
            type
                .GetProperties(BindingFlags.Public | BindingFlags.Instance)
                .Select(info =>  (info, Attribute: info.GetCustomAttribute<TAttribute>()))
